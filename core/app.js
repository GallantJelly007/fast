...
class Middle{
     ...
     * @returns {Promise}
     * @desc Рендерит HTML-шаблон .twig и возвращает его через Promise
     */
    render(path,param=null){
        return new Promise((resolve,reject)=>{
            ...
            Twig.renderFile(path, param, (err, html) => { 
                if(err) reject(err);
                resolve(html);
            });
        })
...
class HttpClient extends Middle{
     ...
     * @param {Object} param 
     * Объект с параметрами которые используется для подстановки в шаблон
     * @desc Рендерит HTML-шаблон .twig и отправляет клиенту
     */
...
class Token{
    ...
    verifyUserToken(user,token,rToken=null) {
        try {
            ...
            if (!user.hasOwnProperty('userId')||!user.hasOwnProperty('keyToken')||!user.hasOwnProperty('keyRtoken')) return false; 
            if(infoAccess.body.data.userId!=user.userId) return false;
            let result = this.verify(token,user.keyToken);
            if(result.success==0){
